  ...
  for (int disorder = 0; disorder < NDisorder; disorder++) {
    ...
    for (int randV = 0; randV < NRandomV; randV++) {
      ...
      kpm0.v.col(0) =
        ...
        kpm0.v.col(
          0
        ); // This factor is due to the fact that this Velocity operator is not self-adjoint
      kpm0.empty_ghosts(0);
      ...
      for (int m = 0; m < N_moments; m += 2) {
        ...
        tmp.setZero();
        constexpr std::size_t nambu = 1 + pairing::is_bdg;
        for (std::size_t ii = 0; ii < r.Sized * nambu; ii += r.Ld[0])
          tmp += kpm0.v.block(ii, 0, r.Ld[0], 1).adjoint() *
                 kpm1.v.block(ii, 0, r.Ld[0], 2);

        gamma.matrix().block(0, m, 1, 2) +=
          (tmp - gamma.matrix().block(0, m, 1, 2)) / value_type(average + 1);
      }
      average++;
    }
  }
...
template <typename T, unsigned D>
void Simulation<T, D>::store_gamma1D(
...
#define instantiate(type, dim)                                                 \
  template void Simulation<type, dim>::                                        \
    Gamma1D(int, int, int, std::vector<std::vector<unsigned>>, std::string);   \
  template void Simulation<                                                    \
    type, dim>::store_gamma1D(Eigen::Array<type, -1, -1> *, std::string);
