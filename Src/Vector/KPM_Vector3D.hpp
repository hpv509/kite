...
private:
  ...
  std::size_t
    *MemIndBeg[3]
              [2]; // Corner from the postion read the boundary for each Orbital
  std::size_t *MemIndEnd
    [3][2]; // Corner from the position to write the boundary for each Orbital
...
public:
  ...
  template <
    int S,
    typename Derived> // S = -1: partition -> lattice, S = 1: lattice -> partition
