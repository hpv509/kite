...
def cuprate(t, r, t_prime):
    lat = latt.Lattice(a1=[1, -1] / np.sqrt(2), a2=[1, 1] / np.sqrt(2))
    lat.add_sublattices(
        ("d", [0.0, 0.0], 0.0),
        ("px", [0.5, -0.5] / np.sqrt(2), 0.0),
        ("py", [0.5, 0.5] / np.sqrt(2), 0.0),
    )
    hop_plus = (t + 1j * r) / 2
    hop_minus = (-t + 1j * r) / 2
    lat.add_hoppings(
...
def main():
    ...
    struc_disorder_D = kite.StructuralDisorder(lattice, position=imp_posA)
    struc_disorder_D.add_vacancy("px")

    # conc = 0.25 / 100.0
    # struc_disorder_A = kite.StructuralDisorder(lattice, concentration=conc)
    # struc_disorder_A.add_vacancy('d')
    # struc_disorder_B = kite.StructuralDisorder(lattice, concentration=conc)
    # struc_disorder_B.add_vacancy('px')
    # struc_disorder_C = kite.StructuralDisorder(lattice, concentration=conc)
    # struc_disorder_C.add_vacancy('py')

    disorder_structural = [struc_disorder_D]
    mode = "open"
    ...
    output_file = f"Data/LocalP_L{size:03d}_F{flag:02d}_O{orbt:02d}_Posy{posy:03d}.h5"
    kite.config_system(
        lattice,
        configuration,
        calculation,
