...
def damping_schedule(n1, gamma1, n2, gamma2):
    """Invert the two-anchor condition for the weighted self-consistency.

    The averaged order parameter obeys

        Delta_n = Delta_{n-1} + gamma_n [ F(Delta_{n-1}) - Delta_{n-1} ],

    with the damping factor gamma_n = w_n / W_n generated by the weights
    w_n = prod_{j<=n} (1 + r j^{-alpha}).  Writing u_n = 1 / gamma_n and
    beta_n = u_n - 1 (the accumulated history in units of the current
    weight), the adiabatic inversion of the recursion gives

        r beta_n = (n - beta_n)^alpha .

    Imposing this at the two anchors (n1, gamma1) and (n2, gamma2) and
    eliminating r yields

        alpha = ln(beta2 / beta1) / ln[(n2 - beta2) / (n1 - beta1)],
        r     = (n1 - beta1)^alpha / beta1 .

    Parameters
    ----------
    n1, n2 : int
        Iteration indices of the two anchors, with n1 < n2.
    gamma1, gamma2 : float
        Requested damping factors at those iterations, with gamma1 > gamma2.
        Each must satisfy 1/n <= gamma < 1, since beta_n <= n - 1 bounds the
        damping from below by 1/n: no choice of (r, alpha) decays faster.

    Returns
    -------
    (r, alpha) : tuple of float
    """
    n1, n2 = int(n1), int(n2)
    gamma1, gamma2 = float(gamma1), float(gamma2)

    if n1 < 2:
        raise SystemExit(
            'The first anchor requires n1 >= 2: the recursion is initialised '
            'with u_1 = 1, i.e. gamma_1 = 1 is fixed by construction.'
        )
    if n2 <= n1:
        raise SystemExit('The anchors must be ordered, n1 < n2.')
    if not 0.0 < gamma2 < gamma1 < 1.0:
        raise SystemExit(
            'The damping factors must satisfy 0 < gamma2 < gamma1 < 1.'
        )
    if gamma1 < 1.0 / n1 or gamma2 < 1.0 / n2:
        raise SystemExit(
            'The damping is bounded from below by gamma_n >= 1/n. Requested '
            'gamma1={:g} at n1={:d} (limit {:g}) and gamma2={:g} at n2={:d} '
            '(limit {:g}).'.format(
                gamma1, n1, 1.0 / n1, gamma2, n2, 1.0 / n2
            )
        )

    beta1 = 1.0 / gamma1 - 1.0
    beta2 = 1.0 / gamma2 - 1.0

    alpha = np.log(beta2 / beta1) / np.log((n2 - beta2) / (n1 - beta1))
    r = (n1 - beta1) ** alpha / beta1

    if alpha > 1.0:
        warnings.warn(
            'The two-anchor inversion returned alpha={:g} > 1. The memory '
            'window beta_n / n ~ n^(alpha-1) / r then grows with n, so the '
            'initial transient is never forgotten and the adiabatic estimate '
            'used here is no longer valid. Increase gamma2 or decrease '
            'n2.'.format(alpha),
            UserWarning,
        )

    return float(r), float(alpha)
...
class Calculation:
    ...
    def s_wave(
        ...
        anchor_1, anchor_2 : tuple of (int, float)
            The damping schedule is specified as "gamma should be gamma1 at
            step n1 and gamma2 at step n2", with n1 < n2 and gamma1 > gamma2.
            The amplitude r and exponent alpha are obtained from these by
            `damping_schedule`.
        prev_iterations : int
            Iterations already completed in a previous run. The stored damping
            state is restored so a restart continues the same schedule rather
            than restarting it.
        """
        weight_r, weight_alpha = damping_schedule(*anchor_1, *anchor_2)
        self._s_wave.append(
            {
    ...
    def s_wave_clean(self, num_random, num_iterations,
                     anchor_1, anchor_2, prev_iterations=0):
        weight_r, weight_alpha = damping_schedule(*anchor_1, *anchor_2)
