...
def sigmoid_weight_schedule(N0, tau, total_iterations=None):
    """
    Parameters
    ----------
    N0 : float
        Iteration at the center of the crossover. May be float('inf').
    tau : float
        Crossover width; sets the plateau gamma_inf = 1 - exp(-1/tau).
    total_iterations : int, optional
        num_iterations + prev_iterations for the run being configured.
        Unused for now (kept for call-site compatibility / future use).

    Returns
    -------
    (N0, tau) : tuple of float
    """
    N0, tau = float(N0), float(tau)
    if N0 <= 0.0:
        raise SystemExit('N0 must be positive.')
    if not tau > 0.0:
...
def _softplus(z):
    """log(1+exp(z)), stable for any z (no overflow)."""
    return max(z, 0.0) + np.log1p(np.exp(-abs(z)))


def sigmoid_u_init(prev_iterations, N0, tau):
    prev_iterations = int(prev_iterations)
    if prev_iterations <= 0:
        return 0.0
    never_freeze = np.isinf(N0)
    rho_ramp = np.exp(1.0 / tau)
    u = 0.0
    for itr in range(1, prev_iterations + 1):
        if never_freeze:
            rho_n = rho_ramp
        else:
            x_prev = (float(itr - 1) - N0) / tau
            x_curr = (float(itr) - N0) / tau
            rho_n = np.exp(_softplus(-x_prev) - _softplus(-x_curr))
        u = 1.0 + u / rho_n
    return float(u)
